...
public class Chennai_Adayar5 {
	...
	public void verifyAdayar_Registeration() throws InterruptedException, IOException {
		...
		for (int i = 1; i < 3; i++) {
			...
			XSSFSheet[] sheets = new XSSFSheet[options31.size()]; // Adjust the size to match the number of options

			int index = 0;
			for (WebElement option : options31) {
				System.out.println(option.getText());
				sheets[index] = workbook.createSheet(option.getText());
				index++;
			}
...
//				}
			...
			FileOutputStream fileOut = new FileOutputStream("data" + sheet++ + ".xlsx");
